<script>
  ...
  export let durationSeconds; // initial seconds from server
  export let onSaved;
  export let onClosed;

  let minutes = Math.floor(durationSeconds / 60);
  let seconds = durationSeconds % 60;
  let hasData = durationSeconds > 0;
...
<div class="overlay" on:click={onClosed} on:keydown={(e) => { if (e.key === 'Escape') onClosed(); }}>
  <div class="dialog" on:click|stopPropagation>
    ...
    <div class="inputs">
      <label>
        Minutes
        <input type="number" bind:value={minutes} min="0" max="999" autofocus on:focus={(e) => e.target.scrollIntoView({ block: 'center' })} />
      </label>
      <label>
        Seconds
